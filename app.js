...
function printLearningSheet() {
  ...
  const koalaStickerUrl = new URL('app-icon.svg', location.href).href;
  ...
  printWindow.document.write(`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>${escapeHtml(context.taskName)} - 练习题</title><style>@page{size:A4;margin:15mm}*{box-sizing:border-box}body{margin:0;color:#17223f;font-family:"Microsoft YaHei",Arial,sans-serif;font-size:15px;line-height:1.75}.screen-actions{display:flex;justify-content:space-between;gap:10px;margin-bottom:12px}.back,.print-button{display:inline-block;padding:8px 13px;color:#253c83;text-decoration:none;border:1px solid #b9c7ef;border-radius:10px;background:#f0f4ff;font:700 14px "Microsoft YaHei",Arial,sans-serif}.print-button{color:#fff;border-color:#3f61c7;background:#4e6ed0;cursor:pointer}.header{display:flex;align-items:center;justify-content:space-between;gap:16px;padding-bottom:10px;border-bottom:2px solid #24396f}.header h1{margin:0;font-size:25px}.header p{margin:3px 0 0;color:#5c6680;font-size:12px}.worksheet-koala{width:58px;height:58px;object-fit:contain;flex:0 0 auto;border-radius:16px}.student{display:flex;gap:32px;margin:14px 0 18px;font-size:14px}.student span{display:inline-block;min-width:140px;border-bottom:1px solid #5a6277}.question{break-inside:avoid;margin:0 0 22px;padding:15px;border:1px solid #cfd7eb;border-radius:10px}.meta{color:#4e66ba;font-size:12px;font-weight:700}.question h2{margin:4px 0 10px;font-size:19px}.content{white-space:normal}.question img{display:block;max-width:100%;max-height:145mm;margin:14px auto 0;border:1px solid #d6dceb}.answer-title{margin-top:17px;font-weight:700}.answer-lines i{display:block;height:25px;border-bottom:1px solid #9aa4bb}@media print{.print-button{display:none}.question{border-color:#9ca7bd}}</style></head><body><nav class="screen-actions"><a class="back" href="${escapeHtml(returnUrl)}">← 返回任务</a><button id="startPrint" class="print-button" type="button">🖨️ 调起系统打印</button></nav><header class="header"><div><h1>${escapeHtml(context.taskName)} · 练习题</h1><p>${escapeHtml(context.subject)} · ${date} · 本页仅含题目，不含答案</p></div><img class="worksheet-koala" src="${escapeHtml(koalaStickerUrl)}" alt="考拉贴纸"></header><div class="student">姓名：<span></span> 日期：<span></span></div>${cards}</body></html>`);
  printWindow.document.close();
  printWindow.focus();
  printWindow.document.getElementById('startPrint')?.addEventListener('click', () => printWindow.print());
}
